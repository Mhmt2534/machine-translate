...
(async () => {
  ...
  try {
    ...
    const response = await fetch(`http://127.0.0.1:${port}/api/translate`, {
      ...
      body: JSON.stringify({ imageId: 'image-1', blocks: [{ id: 'b1', text: 'HELLO' }] }),
    });
    assert.equal(response.status, 503);
    const body = await response.json();
    assert.equal(body.error.code, 'API_KEY_MISSING');
    assert.doesNotMatch(JSON.stringify(body), /Bearer|test-key/);
    console.log('PASS: localhost-only translation server, health, restricted CORS, missing API key error.');
  } finally {
    child.kill();
  }
