...
(async () => {
  ...
  const translationServer = http.createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      const translations = request.blocks.map(block => ({
        id: block.id,
        translatedText: block.text.includes('HELLO') ? 'MERHABA DÜNYA' : 'ŞİMDİ DAHA İYİ HİSSEDİYOR MUSUN?',
        skip: false,
      }));
  ...
  try {
    ...
    await popup.waitForFunction(() => !document.querySelector('#translate-text').disabled, null, { timeout: 10000 });
    ...
    console.log(`PASS: real MV3 extension OCR and popup translation (${translationCalls} context batches), local WASM/eng, cross-origin acquisition, grouping and status.`);
  } finally {
    await context?.close();
